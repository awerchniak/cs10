...
public class BinaryTree<E> {
  private BinaryTree<E> left, right, parent;  // children can be null; parent null for root
  private E data;
  ...
  public BinaryTree(E data) {
    ...
    this.left = null;
    this.right = null;
    this.parent = null;	//set null because first is the root
  }
  ...
  public BinaryTree(E data, BinaryTree<E> left, BinaryTree<E> right) {
    ...
    setLeft(left);			//let other functions deal with setting left/right parent pointers
    setRight(right);
  }
  ...
  public int depth() {
  	//depth of the root is zero. if it's not the root, add 1
  	//recursion :)
  	if(isRoot())
  		return 0;
  	else return 1 + (isRoot() ? 0: parent.depth());
  }
  ...
  public int countLeaves() {
  	//if it's the leaf, return one. otherwise, return sum of left+right countleaves
  	//recursion :)
  	if(isLeaf())
  		return 1;
  	else{
  		if(hasLeft() && hasRight())
  			...
  			System.err.println("Something strange has occured");
  			return -1;
  		}
  	}
  }
  ...
  public BinaryTree<E> copy() {
  	//start at the root and copy until leaves
  	//if it's not a leaf, return a new tree that's the same
  	//recursion :)
  	if(isLeaf()){
  		return this;
  	} else{
  		if(hasLeft() && hasRight())
  			return (new BinaryTree<E>(getValue(), left.copy(), right.copy()));
  		else if(hasLeft())
  			return (new BinaryTree<E>(getValue(), left.copy(), null));
  		else if(hasRight())
  			return (new BinaryTree<E>(getValue(), null, right.copy()));
  		else{
  			System.err.println("Something strange has occurred");
  			return null;
  		}
  	}
  }
